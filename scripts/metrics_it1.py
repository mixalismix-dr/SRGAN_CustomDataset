...
def process_category(category_dir, sr_path, lr_path):
    ...
    for hr_img in tqdm(hr_images, total=len(hr_images), desc=f"Processing {os.path.basename(category_dir)}"):
        ...
        sr_filename = f"res_{tile_name}_down.tif"
        lr_filename = f"{tile_name}_down.tif"
        ...
        if not sr_candidates or not lr_candidates:
            print(f"Skipping {tile_name}: SR or LR image not found")
            continue
...
def main():
    ...
    hr_path = r"C:\Users\mike_\OneDrive\Desktop\MSc Geomatics\Master Thesis\Codebases\SRGAN_CustomDataset\test_data\p3"
