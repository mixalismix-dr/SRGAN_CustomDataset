...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_feat = 3, n_feats = 64, kernel_size = 3, act = nn.LeakyReLU(inplace = True), num_of_block = 3, patch_size = 96):
        ...
        self.conv_final = nn.Conv2d(n_feats * (2 ** num_of_block), 1, kernel_size = 3, stride = 1, padding = 1)
    ...
    def forward(self, x):
        # print(f"Input to Discriminator: {x.shape}")
        
        x = self.conv01(x)
       # print(f"After conv01: {x.shape}")

        x = self.conv02(x)
        # print(f"After conv02: {x.shape}")

        x = self.body(x)
        # print(f"Before view: {x.shape}")  # Debugging: print before view operation
